...
async function callLocalLLM(prompt, session = null) {
  ...
  // 1. 优先尝试本地 LM Studio (通过 SSH 隧道连接 localhost:1234)
  const localUrl = 'http://localhost:1234/v1';
  const localModel = process.env.LOCAL_LLM_MODEL || 'qwen3-4b-instruct-2507';
  
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3000); // 3秒快速检测本地是否在线
    ...
    if (response.ok) {
      ...
      console.log('✅ 已使用本地算力 (LM Studio)');
      if (session) session.log('LLM', 'LocalChat', { model: localModel, durationMs: Date.now() - startTime });
      return content;
    }
  } catch (err) {
    // console.log('⚠️ 本地 LLM 不在线或超时，切换至云端备用...');
  }

  // 2. 本地失败，切换至智谱云端 (GLM-4)
  const cloudUrl = 'https://open.bigmodel.cn/api/paas/v4/chat/completions';
  const cloudModel = 'glm-4'; // 固定使用强大的 GLM-4
  const apiKey = process.env.GLM_API_KEY;

  if (!apiKey) {
    throw new Error('本地 LLM 未连接，且未配置 GLM_API_KEY，无法提供服务');
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`云端 API 错误: ${response.status} ${errorText}`);
    }

    const data = await response.json();
    const content = data.choices[0].message.content;
    console.log('☁️ 已使用云端算力 (ZhipuAI)');
    if (session) session.log('LLM', 'CloudChat', { model: cloudModel, durationMs: Date.now() - startTime });
    return content;
  } catch (err) {
    console.error('[LLM] 云端调用失败:', err.message);
    throw err;
  }
}
...
async function vectorSearch(semanticQuery, limit = 100, session = null) {
  ...
  try {
    ...
    const results = await vectordb.semanticSearch(queryEmbedding, limit);
...
export default async function spatialRoutes(fastify) {
  ...
  fastify.post('/query', async (request, reply) => {
    ...
    try {
      ...
      // 3. 执行向量检索（如果有语义需求）
      if (hasSemanticQuery && vectordb.isMilvusAvailable()) {
...
请给出简洁、友好的回答：`;
  ...
  fastify.get('/status', async () => {
    return {
      postgis: true,
      milvus: vectordb.isMilvusAvailable(),
    };
  });
}
