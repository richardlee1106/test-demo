...
8.  **地理空间布局与中心型展示 (Geo-Spatial Layout)**：
    *   将“绘制圆形”重命名为“绘制圆形（中心型）”。
    *   在圆形模式下，标签云布局不再随机，而是**严格保持 POI 的地理方位**（上北下南左西右东）。
    *   标签云中心显示“中心位置”文本，以橙黄色高亮，对应地图上的圆心位置。
9.  **标签云布局紧凑化与防重叠优化 (Layout Compactness & Collision)**：
    *   解决地理空间布局模式下，标签之间缝隙过大及密集区域严重重叠问题。
    *   实现自定义矩形碰撞检测，引入**多阶段强力碰撞解决机制**。
    *   平衡“紧凑性”与“无重叠”，支持密集 POI 簇的自然展开。
...
### 2.8 地理空间布局 (Geo-Spatial Layout)

**技术栈**：Web Workers, D3-Force, Coordinate Projection

**实现逻辑**：

1.  **坐标转换与传递**：
    *   `MapContainer.vue` 在圆形绘制完成后，将圆心坐标从 Web Mercator (EPSG:3857) 转换为经纬度 (Lon/Lat)，通过 `circleCenter` 参数向上传递。
    *   `TagCloud.vue` 识别到 `drawMode === 'Circle'` 时，优先实例化 `GeoWorker`。
    *   **数据清洗**：传递给 Worker 的数据（Tags 数组）必须经过 JSON 序列化/反序列化 (`JSON.parse(JSON.stringify(tags))`)，去除 Vue 的 Proxy 代理对象，防止 `postMessage` 克隆错误。
2.  **GeoWorker 布局算法**：
    *   **中心点注入**：在 Worker 中手动创建一个虚拟的“中心位置”节点，固定在画布中心 (`width/2, height/2`)。
    *   **均匀投影 (Uniform Projection)**：
        *   使用 `Math.cos(centerLat)` 作为经度收缩因子，模拟近似的米级距离，消除高纬度下的拉伸变形。
        *   计算所有标签相对于中心点的最大偏移量 (`maxDx`, `maxDy`)。
        *   采用统一缩放比例 (`scale = min(w/dx, h/dy)`)，确保长宽比一致，**严格保持地理方位**。
    *   **自定义矩形碰撞检测 (Custom Rectangular Collision)**：
        *   放弃 D3 自带的 `forceCollide`（圆形检测导致矩形标签间隙过大）。
        *   实现自定义 `rectCollide` 函数，基于 AABB（轴对齐包围盒）进行碰撞检测。
        *   **优化策略**：
            *   **微调 Padding**：设置 `padding = 1`，在保证无重叠的同时，维持视觉上的紧凑感。
            *   **弱化引力**：将回归原位的引力 (`forceX/Y`) 强度降至 `0.05`，允许标签为了避让而大幅度偏离原位。
            *   **多阶段碰撞解决**：
                1.  **模拟阶段 (Simulation)**：增加至 300 帧。每帧运行 6 次碰撞迭代，使密集区域快速散开。
                2.  **冷却阶段 (Cleanup)**：在模拟结束后，额外运行 120 次“静态碰撞解决”，完全关闭引力，强制移除所有残余重叠。
                3.  **随机抖动 (Jitter)**：在初始位置添加微小随机扰动，防止完全重叠的点卡死。
...
*   **原因**：简单的经纬度差值映射未考虑地球曲率，经度圈随纬度升高而收缩。
*   **解决**：引入校正因子 `factorX = Math.cos(centerLat)`，对经度差值进行加权，恢复真实的视觉比例。

### 问题 5：Worker 数据传递报错 (DataCloneError)
*   **现象**：`Uncaught DOMException: Failed to execute 'postMessage' on 'Worker': [object Array] could not be cloned.`
*   **原因**：Vue 3 的响应式对象（Proxy）包含不可序列化的内部属性，无法直接通过 `postMessage` 传递给 Web Worker。
*   **解决**：在传递前使用 `JSON.parse(JSON.stringify(tags))` 进行深拷贝，剥离 Proxy 包装。

### 问题 6：密集区域标签重叠严重
*   **现象**：在 POI 密集的商圈，标签云即使经过布局，仍有约 1/3 的区域发生文字重叠。
*   **原因**：
    1.  D3 力导向图的引力过强，将标签强行拉回重叠位置。
    2.  单帧碰撞检测迭代次数不足，无法解算复杂的多重重叠。
    3.  缺乏最终的强制清理阶段。
*   **解决**：
    1.  **降低引力**：将 `strength` 从 0.1 降至 0.05。
    2.  **增加迭代**：将单帧碰撞检测次数从 2 提升至 6，总模拟帧数从 200 提升至 300。
    3.  **强制清理**：新增 120 帧纯碰撞检测阶段（无引力），像“挤泡泡”一样将重叠标签彻底推开。
