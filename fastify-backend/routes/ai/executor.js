...
async function execBasicMode(plan, frontendPOIs, options = {}) {
  ...
  if (plan.semantic_query && milvus.isMilvusAvailable() && candidates.length > 0) {
...
async function execGlobalContextMode(plan, frontendPOIs, options = {}) {
  ...
  basicResult.mode = 'global_context'
  
  // 计算区域画像
  if (frontendPOIs.length > 0) {
    basicResult.area_profile = computeAreaProfile(frontendPOIs)
  } else if (basicResult.anchor) {
    // 从数据库计算（如果有锚点但没有前端数据）
    basicResult.area_profile = await computeAreaProfileFromDB(
      basicResult.anchor,
...
async function semanticRerank(candidates, semanticQuery, topK) {
  try {
    ...
    const semanticResults = await milvus.semanticSearch(embedding, topK * 2, candidateIds)
 ...
 * 从数据库计算区域画像
 */
async function computeAreaProfileFromDB(anchor, radius) {
  try {
    // 调用数据库聚合查询
    const profile = await db.getCategoryStats(anchor, radius)
    return profile
  } catch (err) {
